...
# Fla alohida thread-da ishga tushirish
...
async def send_topup_menu(message, price_needed):
    await message.answer(
        f"❌ Sizning balansingizda yetarli mablag' yo‘q. Raqam narxi: {price_needed} so‘m.\n\n"
...
async def send_referral(message: types.Message):
    ...
    await message.answer(
        ...
        "Do‘stlaringiz ushbu havola orqali start bersa, siz va do‘stingiz 3000 so‘m balansga ega bo‘lasiz!",
        reply_markup=ReplyKeyboardBuilder()
            .row(KeyboardButton(text="⬅️ Ortga"))
            .as_markup(resize_keyboard=True)
    )
